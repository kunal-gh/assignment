...
class RankingEngine:
    ...
    def __init__(
        self, semantic_weight: float = 0.7, skill_weight: float = 0.3, embedding_generator: Optional[EmbeddingGenerator] = None
    ):
        """
    ...
    def calculate_semantic_score(self, resume_embedding: np.ndarray, job_embedding: np.ndarray) -> float:
        ...
        if resume_embedding.shape != job_embedding.shape:
            logger.error(f"Embedding dimension mismatch: {resume_embedding.shape} vs {job_embedding.shape}")
            return 0.0
    ...
    def calculate_hybrid_score(self, resume: ResumeData, job_desc: JobDescription) -> Dict[str, float]:
        ...
        try:
            ...
            semantic_score = self.calculate_semantic_score(resume.embedding, job_desc.embedding)
            ...
            hybrid_score = self.semantic_weight * semantic_score + self.skill_weight * skill_score
            ...
            return {"semantic_score": semantic_score, "skill_score": skill_score, "hybrid_score": hybrid_score}

        except Exception as e:
            logger.error(f"Error calculating hybrid score: {str(e)}")
            return {"semantic_score": 0.0, "skill_score": 0.0, "hybrid_score": 0.0}
    ...
    def rank_candidates(self, resumes: List[ResumeData], job_desc: JobDescription) -> List[RankedCandidate]:
        ...
        for resume in resumes:
            try:
                ...
                error_candidate = RankedCandidate(resume=resume, semantic_score=0.0, skill_score=0.0, hybrid_score=0.0, rank=0)
    ...
    def explain_ranking(self, candidate: RankedCandidate, job_desc: JobDescription) -> str:
        ...
        try:
            ...
            # Overall score
            explanation_parts.append(f"Overall match score: {candidate.hybrid_score:.1%} " f"(Rank #{candidate.rank})")
            ...
            if job_desc.matches_experience_level(years_exp):
                explanation_parts.append(f"Experience level ({years_exp} years) matches job requirements.")
            else:
                explanation_parts.append(
    ...
    def process_batch(
        self, resumes: List[ResumeData], job_desc: JobDescription, include_fairness: bool = True
        ...
        try:
            ...
            successfully_parsed = sum(1 for resume in resumes if resume.contact_info and resume.contact_info.name != "Unknown")
    ...
    def get_ranking_stats(self, candidates: List[RankedCandidate]) -> Dict[str, Any]:
        ...
        return {
            ...
            "skill_scores": {"mean": np.mean(skill_scores), "median": np.median(skill_scores), "std": np.std(skill_scores)},
            "weights": {"semantic_weight": self.semantic_weight, "skill_weight": self.skill_weight},
        }
