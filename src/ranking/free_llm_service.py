...
Explanation:"""
    ...
    def _generate_template_explanation(
        ...
        if semantic_score >= 0.7:
            ...
            explanation_parts.append(
                f"Moderate semantic match ({semantic_score:.1%}) shows some relevant experience."
            )
        else:
            explanation_parts.append(
                f"Limited semantic alignment ({semantic_score:.1%}) suggests different background focus."
            )
...
class FreeLLMServiceSync:
    ...
    def generate_explanation(self, **kwargs) -> str:
        ...
        return loop.run_until_complete(
            self.service.generate_explanation(**kwargs)
        )
