...
def handler(request):
    ...
    try:
        ...
        if not job_description:
            return {"statusCode": 400, "headers": headers, "body": json.dumps({"error": "Job description is required"})}
        ...
        engine = RankingEngine(
            semantic_weight=semantic_weight, skill_weight=1.0 - semantic_weight, embedding_generator=generator
        )
        ...
        # Process batch
        results = engine.process_batch(resumes=resumes, job_desc=job_desc, include_fairness=include_fairness)
